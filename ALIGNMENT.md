...
1. **本轮范围**：只做 **P0（正确性，纯修 bug，风险最低）**，还是 **P0+P1（正确性 + EV 决策语义，一次性把判定核心换对）**？P2 编排深度（审计/收敛/成本归因/worker 注入）是否另开一轮？
2. **版本号策略**：本项目保持 `0.x` 并只增加「上游对齐版本」字段，还是跳到 `1.0.0` 宣告与上游 1.x 对齐？
3. **EV 是替换还是并存**：上游是「替换 + 旧默认值静默迁移为 dead」。本项目现在是置信度加权比例窗口，是否照此**替换**（推荐，两套语义并存会长期污染文档与测试）？
4. **`/router on|off` 持久性**：保持会话级（DSH 语义自洽）+ 文档写明，还是改为写 settings（对齐上游）？
5. **worker 模型注入（C4）**：接受「需在 DSH 的 `subagent-model-selection` 中授权 Fast 链」这个前置条件吗？若是，选 (a) 卡片代写该 namespace / (b) 文档化 + 启动自检 / (c) 接受部署侧固定模型并如实改提示词。
6. **skill 0.4.0**：是否把未提交的 `VERSION` + frontmatter 同步到已安装目录并推到远端 `main`（当前远端只有 0.3.0）？
7. **SDK 升级（E5）**：是否把 `0.1.0-rc.6 → 0.1.5-rc.2` 列为对齐动作的**前置步骤**？
