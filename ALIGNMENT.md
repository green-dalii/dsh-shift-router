...
| 缺陷 | 根因 | 修法 |
|---|---|---|
| 行角色徽标压住 provider 下拉 | 行网格第一列写死 `20px`，而 R7 把原本一位数字换成了 `主选` / `备选 1` 文本；grid item 超出轨道不会裁剪，直接盖到下一列 | 该列改为按内容定宽（`auto`），并把整行从 grid 改成 flex：徽标 `flex:none`、两个控件 `flex:1 1 0; min-width:0`、操作为 `flex:none` |
| 单位文字（`ms`/`tokens`/…）压住数字输入 | 单位是绝对定位在输入框内部的（`right:12` + `paddingRight:54`），R7 把输入改成 `type=number` 后，浏览器在右端画出的步进箭头正好落在单位文字上 | 单位移出输入框，作为普通 flex 兄弟节点（`gap:6`）；删掉 `position:absolute` 与 `paddingRight` 补丁 |

**这才是这类缺陷的正确闸门**：布局问题无法用单测表示，但**可以用包围盒测出来**。
新增的 `e2e/browser-check.mjs` 就是把它变成可重复执行的检查（见 R8.4）。
...
| Gate | 内容 | 变异自检 |
|---|---|---|
| `e2e/browser-check.mjs`（可选、需浏览器） | 真机打开卡片的插件配置页：卡片存在且可展开；加两行模型后**包围盒两两相交数为 0**；provider 下拉里至少有 1 个来自部署的选项（目录真的加载了）；高级区默认折叠、展开后控件数 ≥ 15；可选输出截图 | 把行网格改回 `20px`、或把单位放回输入框内部 → **相交数 > 0，红** |
